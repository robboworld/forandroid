...
public class GestureHandler {
    ...
    private const LONG_CLICK_MSECS:int = 800;
	...
	public function mouseDown(evt:MouseEvent):void {
		...
		//trace("target", t);
		//trace("mouse target", mouseTarget);
		if (mouseTarget == null) {
			gesture = "ignore";
			return;
		}
	...
	public function mouseUp(evt:MouseEvent):void {
		//trace("mouse up tar", mouseTarget);
		if (gesture == "debug") { evt.stopImmediatePropagation(); return; }
		mouseIsDown = false;
		//trace("drag client", dragClient);
		if (dragClient != null) {
	...
	private function findMouseTarget(evt:MouseEvent, target:*):DisplayObject {
		...
		while (o != null) {
			if (isMouseTarget(o, evt.stageX, evt.stageY)) {
				mouseTarget = true;
				break;
			}
			o = o.parent;
    ...
    private function handleDoubleClick(evt:MouseEvent):void {
        ...
        if ('menu' in mouseTarget && mouseTarget is Block) {
            var blockTarget:Block = mouseTarget as Block;
            if ((blockTarget.op == Specs.GET_LIST || blockTarget.op == Specs.GET_VAR) && blockTarget.isInPalette()) {
                handleMenu(evt);
                return;
            }
        }
	...
	private function dropHandled(droppedObj:*, evt:MouseEvent):Boolean {
		...
		if(app.isIn3D) app.stagePane.visible = true;
		var possibleTargets:Array = stage.getObjectsUnderPoint(new Point(evt.stageX, evt.stageY));
		if(app.isIn3D) {
	...
	private function drop(evt:MouseEvent):void {
		//trace("drop", evt.target, evt.stageX, evt.stageY);
