...
public class TabsPart extends UIPart {
	...
	private const titleTextFormat:TextFormat = new TextFormat(CSS.font, 16, CSS.textColor, true);
	...
	public function TabsPart(app:Scratch) {
		...
		connectionStatus = makeLabel("", titleTextFormat, 0, 0);
		connectionStatus.height = scriptsTab.height;
		addChild(scriptsTab);
		addChild(imagesTab);
		addChild(soundsTab);
	...
	public function refresh():void {
		...
		if (app.device != null && app.device.isConnected())
			connectionStatus.text = " " + Translator.map("Connected to ") + app.device.getName();
		else
	...
	public function fixLayout():void {
		...
		soundsTab.y = 0;
		connectionStatus.x = soundsTab.x + soundsTab.width + 1;
		connectionStatus.y = 0;
		//this.w = soundsTab.x + soundsTab.width;
		this.w = connectionStatus.x + connectionStatus.width;
