package connectors
{
	...
	public class AndroidBluetooth implements IConnector
	{
		...
		//public function btInitiated( b:BluetoothEvent ):void{ trace('BTInit: ' + b.toString() ); }
		...
		public function AndroidBluetooth() {
			if (Bluetooth.isSupported()) {
				...
				/*bluetooth.showSettings();
				var par:Vector.<BluetoothDevice> = bluetooth.getPairedDevices();
				for each(var t:BluetoothDevice in par)
					trace(t.name, " ", t.address);*/
				...
				switch (b.type)
				{
					case BluetoothScanEvent.BLUETOOTH_DEVICE_FOUND:
						...
						for each (var el:IDevice in availableDevices)
							if (el.address() == b.device.address)
								found = true;
						if (!found)
							availableDevices.push(new AndroidBluetoothDevice(b.device));
						break;
